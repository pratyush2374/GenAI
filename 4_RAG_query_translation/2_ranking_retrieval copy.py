...
def ranked_retrieval(all_chunks):
    ranks = {}
    ...
    sorted_ids = sorted(ranks.items(), key=lambda item: item[1], reverse=True)
    temp = {chunk.metadata.get("_id"): chunk for chunk in all_chunks}
    return [temp[chunk_id] for chunk_id, _ in sorted_ids][:3]
...
while True:
    ...
    ranked_chunks = ranked_retrieval(found_chunks)
